...
{
    public partial class SubjectCodeConfig : BaseForm
    {
        //private List<JHClassRecord> _classes;
        protected BackgroundWorker _worker;
        protected AccessHelper _accessHelper;
        private List<SubjectCode_SubjectMakeUp> _list;
        ...
        {
            _list = _accessHelper.Select<SubjectCode_SubjectMakeUp>();
            //if (_list.Count <= 0)
        ...
        protected void RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            {
                DataGridViewRow row = new DataGridViewRow();
                row.CreateCells(dataGridViewX1, record.Subject, record.Code);
        ...
        protected void Save()
        {
            ...
            List<SubjectCode_SubjectMakeUp> newList = new List<SubjectCode_SubjectMakeUp>();
            foreach (DataGridViewRow row in dataGridViewX1.Rows)
            {
                if (row.IsNewRow) continue;

                SubjectCode_SubjectMakeUp cc = new SubjectCode_SubjectMakeUp();
                cc.Subject = "" + row.Cells[chSubjectName.Index].Value;
        ...
        private void dgv_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                ValidateDuplication(dataGridViewX1, chSubjectName.Index, "科目名稱不能重覆");
            }
        }

        protected void Export()
        {
            ImportExport.Export(dataGridViewX1, "匯出科目代碼設定");
        }

        protected void Import()
        {
            ImportExport.Import(dataGridViewX1, new List<string>() { "科目", "代碼" });
