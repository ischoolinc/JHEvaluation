...
{
    public partial class frmExportStudentDomainScore : BaseForm
    {
        ...
        public frmExportStudentDomainScore(List<string> listIDs)
        {
            InitializeComponent();
        ...
        private void frmExportStudentDomainScore_Load(object sender, EventArgs e)
        {
...
ORDER BY
    ...
    , student.id
        ...
        private void ParseData()
        {
            ...
            List<StudentRecord> StudentRecList = K12.Data.Student.SelectByIDs(listStudentIDs);
            ...
            foreach (DataRow row in dtRsp.Rows)
            {
                ...
                // 資料整理
                {
                    ...
                    if (listDomainName.Contains(domain) && domain != "特殊需求")  //如果領域存在領域資料管理，且不是特殊需求才列印，沒有則忽略-Cynthia 2021.08
                    {
                        ...
                        if (!stuRec.dicScoreByDomainBySchoolYear[ssKey].ContainsKey(domain))
                        {
        ...
        private DataTable FillMergeFiledData()
        {
            ...
            foreach (string id in dicStudentByID.Keys)
            {
                ...
                if (calcIDCache.ContainsKey(id) && calcCache.ContainsKey(calcIDCache[id]))
                    studentCalculator = calcCache[calcIDCache[id]];
                ...
                foreach (string domain in dicStudentByID[id].listDomainFromStu)
                {
                    ...
                    if (listScore.Count > 0)
                    {
                        ...
                        if (totalScore != 0)
                        {
                            decimal avgScore = studentCalculator.ParseGraduateScore(totalScore / domainScoreCount);
                ...
                foreach (string key in dicStudentByID[id].dicSchoolYear.Keys)
                {
                    ...
                    if (listScore.Count > 0)
                    {
                        ...
                        if (totalPower != 0)
                        {
                            decimal score = studentCalculator.ParseGraduateScore(totalScore / totalPower);
                ...
                if (listDomainAvgScore.Count > 0)
                {
                    ...
                    //    totalScore += score;
                    //}
                    decimal avgScore = studentCalculator.ParseGraduateScore(allTotalScore / scoreCount);
