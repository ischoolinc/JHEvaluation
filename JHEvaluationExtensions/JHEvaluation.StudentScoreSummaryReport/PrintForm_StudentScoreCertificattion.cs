...
namespace JHEvaluation.StudentScoreSummaryReport
{
    public partial class PrintForm_StudentScoreCertificattion : BaseForm, IStatusReporter
    {
        ...
        private void MasterWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            #region 學務相關資料 
            //出缺勤紀錄
            table.Columns.Add("上課天數_1");
            table.Columns.Add("上課天數_2");
            table.Columns.Add("上課天數_3");
            table.Columns.Add("上課天數_4");
            table.Columns.Add("上課天數_5");
            table.Columns.Add("上課天數_6");
            ...
            foreach (string stuID in StudentIDs)
            {
                ...
                if (jssr_dict.ContainsKey(stuID))
                {
                                ...
                                foreach (var subjectscore in jssr.Subjects)
                                {
                                    ...
                                    if (subjectscore.Value.Domain != "彈性課程" && subjectscore.Value.Domain != "彈性學習" && subjectscore.Value.Domain != "")
                                    {
                                        if (SubjectCourseDict.ContainsKey(subjectscore.Value.Domain))
                                        {
                                            ...
                                            row[subjectscore.Value.Domain + "_科目名稱" + subjectCourseCount] = subjectscore.Value.Subject;
                    ...
                    if (isExceed)
                    {
                        MessageBox.Show("科目數超過報表變數可支援數量，超過的將不會顯示在在校成績證明書中");
                    }
                }
                            ...
                            if (jssr.SchoolYear == schoolyear_grade_dict[grade])
                            {
                                foreach (var subjectscore in jssr.Subjects)
                                {
                                    ...
                                    if (subjectscore.Value.Domain == "彈性課程" || subjectscore.Value.Domain == "彈性學習" || subjectscore.Value.Domain == "")
                                    {
                                        ...
                                        row["彈性課程_科目名稱" + AlternativeCourse] = subjectscore.Value.Subject;
                                    ...
                                    foreach (var subjectscore in jssr.Subjects)
                                    {
                                        ...
                                        if (subjectscore.Value.Domain == "彈性課程" || subjectscore.Value.Domain == "彈性學習" || subjectscore.Value.Domain == "")
                                        {
                                            if (AlternativeCourseDict.ContainsKey(subjectscore.Value.Subject))
                                            {
                                                ...
                                                {
                                                    subjectLevel_dict["彈性課程_科目" + AlternativeCourse + "_等第" + (grade * 2 - 1)] = ScoreTolevel(subjectscore.Value.Score);
                                                }

                                                //紀錄原始等第
                                                if (subjectLevel_dict.ContainsKey("彈性課程_科目" + AlternativeCourse + "_原始等第" + (grade * 2 - 1)))
                                                {
                                                    subjectLevel_dict["彈性課程_科目" + AlternativeCourse + "_原始等第" + (grade * 2 - 1)] = ScoreTolevel(subjectscore.Value.Score);
                                                }
                                        ...
                                        else
                                        {
                                            if (SubjectCourseDict.ContainsKey(subjectscore.Value.Domain))
                                            {
                                                if (SubjectCourseDict[subjectscore.Value.Domain].ContainsKey(subjectscore.Value.Subject))
                                                {
                                                    ...
                                                    {
                                                        subjectLevel_dict[subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_等第" + (grade * 2 - 1)] = ScoreTolevel(subjectscore.Value.Score);
                                                    }

                                                    //換算原始等第
                                                    if (subjectLevel_dict.ContainsKey(subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_原始等第" + (grade * 2 - 1)))
                                                    {
                                                        subjectLevel_dict[subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_原始等第" + (grade * 2 - 1)] = ScoreTolevel(subjectscore.Value.Score);
                                                    }
                                    ...
                                    foreach (var subjectscore in jssr.Subjects)
                                    {
                                        ...
                                        if (subjectscore.Value.Domain == "彈性課程" || subjectscore.Value.Domain == "彈性學習" || subjectscore.Value.Domain == "")
                                        {
                                            if (AlternativeCourseDict.ContainsKey(subjectscore.Value.Subject))
                                            {
                                                ...
                                                {
                                                    subjectLevel_dict["彈性課程_科目" + AlternativeCourse + "_等第" + (grade * 2)] = ScoreTolevel(subjectscore.Value.Score);
                                                }

                                                //紀錄原始等第
                                                if (subjectLevel_dict.ContainsKey("彈性課程_科目" + AlternativeCourse + "_原始等第" + (grade * 2)))
                                                {
                                                    subjectLevel_dict["彈性課程_科目" + AlternativeCourse + "_原始等第" + (grade * 2)] = ScoreTolevel(subjectscore.Value.Score);
                                                }
                                        ...
                                        else
                                        {
                                            if (SubjectCourseDict.ContainsKey(subjectscore.Value.Domain))
                                            {
                                                if (SubjectCourseDict[subjectscore.Value.Domain].ContainsKey(subjectscore.Value.Subject))
                                                {
                                                    ...
                                                    {
                                                        subjectLevel_dict[subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_等第" + (grade * 2)] = ScoreTolevel(subjectscore.Value.Score);
                                                    }

                                                    //換算原始等第
                                                    if (subjectLevel_dict.ContainsKey(subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_原始等第" + (grade * 2)))
                                                    {
                                                        subjectLevel_dict[subjectscore.Value.Domain + "_科目" + SubjectCourseNum + "_原始等第" + (grade * 2)] = ScoreTolevel(subjectscore.Value.Score);
                                                    }
                ...
                if (urr_dict.ContainsKey(stuID))
                {
                    foreach (K12.Data.UpdateRecordRecord urr in urr_dict[stuID])
                    {
                        ...
                        if (urr.UpdateCode == "1")
                        {
                            ...
                            row["入學日期"] = enterday.ToString("yyyy/MM/dd");
                       ...
                       + EastAsiaNumericFormatter.FormatWithCulture("Ln", (enterday.Month), null, new CultureInfo("zh-tw")) + "月"
                        ...
                        if (urr.UpdateCode == "2")
                        {
                            ...
                            row["畢業年月"] = enterday.ToString("yyyy/MM");
        ...
        {
            //2022 Cynthia
            Global.ExportMappingFieldWord("中文版");
        }
