...
namespace JHSchool.Evaluation.Calculation
{
    public class ScoreCalculator
    {
        ...
        private class Round
        {
            ...
            {
                decimal seed = (decimal)Math.Pow(10, decimals);
                decimal rounded;
                switch (mode)
                {
                    default:
                    case RoundMode.四捨五入:
                        rounded = decimal.Round(score, decimals, MidpointRounding.AwayFromZero);
                        break;
                    case RoundMode.無條件捨去:
                        rounded = decimal.Floor(score * seed) / seed;
                        break;
                    case RoundMode.無條件進位:
                        rounded = decimal.Ceiling(score * seed) / seed;
                        break;
                }
                return rounded;
            }
